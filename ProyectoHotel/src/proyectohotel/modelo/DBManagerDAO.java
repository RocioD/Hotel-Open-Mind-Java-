...
public class DBManagerDAO {
    ...
    public ListaConsumoVO consumos() {
        ListaConsumoVO lista = new ListaConsumoVO();
        int codigo_producto;
        int cantidad_pedida;
        int numero_orden;
        ConsumoVO consumo;
        if (conectar()) {
            try {
                ...
                ResultSet result = st.executeQuery(sql);
                while (result.next()) {
                    codigo_producto = result.getInt("codigo_producto");
                    cantidad_pedida = result.getInt("cantidad_pedida");
                    numero_orden = result.getInt("numero_orden");
                    consumo = new ConsumoVO(codigo_producto, cantidad_pedida, numero_orden);
                    lista.agregarConsumo(consumo);
                }
                desconectar();
                return lista;
            } catch (SQLException ex) {
    ...
    public boolean ingresarPedido(ConsumoVO nuevo) {
        if (conectar()) {
            try {
                Statement st = con.createStatement();
                String sql = "insert into consumo values (" + nuevo.toString() + ")";
                System.out.println("Insertando [" + sql + "]");
                int rowsAffected = st.executeUpdate(sql);
                desconectar();
                return (rowsAffected == 1);
    ...
    public boolean modificarPedido(ConsumoVO nuevo) {
        if (conectar()) {
            try {
                Statement st = con.createStatement();
                String sql = "update consumo set cantidad_pedida = " + nuevo.getCantidad_pedida()
                        + " where codigo_producto = " + nuevo.getCodigo_producto()
                        + " and numero_orden = " + nuevo.getNumero_orden();
    ...
    public boolean existePedido(int codigo_producto, int numero_orden) {
        if (conectar()) {
            try {
                Statement st = con.createStatement();
                String sql = "select * from consumo where codigo_producto = " + codigo_producto
    ...
    public String registros(String rut) { //Entrega los ingresos de un cliente
        String registros = "";
        if (conectar()) {
            try {
                Statement st = con.createStatement();
    ...
    public String productosPorNumeroOrden(int numero_orden) {
        ...
        if (conectar()) {
            try {
                Statement st = con.createStatement();
                String sql = "SELECT codigo_producto, cantidad_pedida from consumo where "
                        + "numero_orden = " + numero_orden;
                ResultSet result = st.executeQuery(sql);
                while (result.next()) {
                    productos = productos + result.getInt("codigo_producto") + "#"
                            + result.getInt("cantidad_pedida") + "\n";
                }
                desconectar();
                return productos;
    ...
    public String registrosDeUnIngreso(String numero_orden) {
        ...
        if (conectar()) {
            try {
                Statement st = con.createStatement();
                String sql = "select H.rut, C.nombres, C.apellido_paterno, C.apellido_materno from clientes C, historico H where numero_orden = "
                        + numero_orden + " and H.rut = C.rut";
                ResultSet result = st.executeQuery(sql);
                while (result.next()) {
                    registros = registros + "Rut: " + result.getString("rut") + "  -  "
    ...
    public int dineroPorUsoHabitacionPorUnaOrden(int numero_orden) {
        ...
        if (conectar()) {
            try {
                Statement st = con.createStatement();
                String sql = "SELECT costo_habitacion from Ingresos where numero_orden = " + numero_orden;
                ResultSet result = st.executeQuery(sql);
                if (result.next()) {
                    dinero = result.getInt("costo_habitacion");
                }
                desconectar();
                return dinero;
            } catch (SQLException ex) {
            ...
            try {
                ...
                while (result.next()) {
                    dineroPorUso = dineroPorUso + "Orden: " + result.getInt("numero_orden")
                            + ", dinero recaudado por estadía: " + result.getInt("costo_habitacion") + "\n";
                }
                desconectar();
                return dineroPorUso;
            } catch (SQLException ex) {
